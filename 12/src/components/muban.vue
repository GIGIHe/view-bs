<template>
  <div class="wraper">
     <el-input v-model.trim="search" class="search" @keyup.enter.native="search_btn"></el-input><el-button type="primary" icon="el-icon-search" @click="search_btn">搜索</el-button>
    <ul>
      <li
        class="item"
      ...
      >
        <img v-lazy="item.poster" alt :title="item.g_desc">
        <span class="title">{{item.g_name}}</span>
        <span class="price">￥{{item.g_price}}</span>
             <p class="car" @click="add(item._id)">加入购物车</p>
      </li>
    </ul>
  </div>
</template>
